...
namespace sb {
  ...
  SpeedbootNerve::SpeedbootNerve(HakoniwaSequence* sequence)
      : al::NerveExecutor("Speedboot"), sequence(sequence),
        // DonsukeExStage:main_exit ArenaCityWorldExStage
        changeStageInfo(sequence->mGameDataHolder, "", "RollingExStage", false, -1, ChangeStageInfo::UNK) {
    initNerve(&SpeedbootNerveNrvLoad::sInstance, 0);
    //        Logger::log("Speedbooting into %s:%s\n", par::get("SpeedyStage", "DonsukeExStage"),
    //        par::get("SpeedyChangeId", "DonsukeExStage"));
    Logger::log("Speedbooting into %s:%s\n", changeStageInfo.changeStageName.cstr(),
                changeStageInfo.changeStageId.cstr());
  }
  ...
  struct SetupSpeedbootNerve : exl::hook::impl::TrampolineHook<SetupSpeedbootNerve> {
    static void Callback(HakoniwaSequence* sequence, const al::SequenceInitInfo& initInfo) {
      ...
      if (!par::get("SpeedyEnabled", false)) {
        Logger::log("skipping speedboot\n");
        return;
      }
