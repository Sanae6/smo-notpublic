...
namespace handler {
    ...
    namespace detail {
        namespace local {
            ...
            static nn::os::TlsSlot catchStackBottom;

            NOINLINE void popCatch() {
                auto* frame = reinterpret_cast<CatchStackFrame*>(nn::os::GetTlsValue(catchStackBottom));
                EXL_ASSERT(frame != nullptr, "Current catch stack frame is null!");
                nn::os::SetTlsValue(catchStackBottom, reinterpret_cast<uintptr_t>(frame->parentFrame));
                free(frame);
            }

            NOINLINE CatchStackFrame* pushCatch(const CatchFunc& func, uintptr_t sp, uintptr_t fp) {
                auto* previousFrame = reinterpret_cast<CatchStackFrame*>(nn::os::GetTlsValue(catchStackBottom));
                auto* newFrame = reinterpret_cast<CatchStackFrame*>(malloc(sizeof(CatchStackFrame)));
                EXL_ASSERT(newFrame != nullptr, "New catch stack frame is null!");
                ...
                nn::os::SetTlsValue(catchStackBottom, reinterpret_cast<uintptr_t>(newFrame));
                return newFrame;
            }
        ...
        static void printStackTrace(const ExceptionInfo* info, nn::diag::ModuleInfo* moduleInfos,
                                    int moduleCount, bool printPCLR = true, int traceLength = 50) {
            if(printPCLR) {
            ...
            while (frame != nullptr && frame != prevFrame) {
                if(traceLength >= 0 && index >= traceLength)
                    break;
        ...
        static void printCrashReport(const ExceptionInfo* info) {
            ...
            printStackTrace(info, moduleInfos, moduleCount);
    ...
    void printStackTrace(const ExceptionInfo& info, int traceLength, bool printPCLR) {
        nn::diag::ModuleInfo* moduleInfos;
        uintptr_t bufSize = nn::diag::GetRequiredBufferSizeForGetAllModuleInfo();
        void* moduleBuffer = alloca(bufSize);
    ...
    bool getAddrModuleName(char* outName, uintptr_t addr) {
        ...
        nn::diag::ModuleInfo containModule = {};
        bool result = detail::findContainModule(containModule, moduleInfos, moduleCount, addr);

        if(result) {
            Logger::log("Module Name: %s\n", containModule.mPath);
            strcpy(outName, containModule.mPath);
        }
