...
static void* glslc_Alloc(size_t size, size_t alignment, void* user_data = nullptr) {
  //    Logger::log("Allocating ptr with size: %x aligned by %x\n", size, alignment);
  return nn::init::GetAllocator()->Allocate(ALIGN_UP(size, alignment));
}
static void glslc_Free(void* ptr, void* user_data = nullptr) {
  //    Logger::log("Freeing ptr: %p\n", ptr);
  nn::init::GetAllocator()->Free(ptr);
}
static void* glslc_Realloc(void* ptr, size_t size, void* user_data = nullptr) {
...
bool ImguiShaderCompiler::CheckIsValidVersion(nvn::Device* device) {
  ...
  if (glslcDll->IsInitialized()) {
    ...
    Logger::log("Current NVN Api is incompatible with supplied GLSLC binary!\nTry using a GLSLC binary from a "
                "different game.\n");
    return false;
