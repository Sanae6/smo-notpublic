...
namespace ImguiNvnBackend {
  ...
  static void ScaleWindow(ImGuiWindow* window, float scale) {
    ...
    window->Pos =
        ImFloor(ImVec2((window->Pos.x - origin.x) * scale + origin.x, (window->Pos.y - origin.y) * scale + origin.y));
  ...
  // doesn't get used anymore really, as back when it was needed i had a simplified shader to test with, but now I just
  ...
  int texIDSelector() {
    {
       ...
       * 282 = Window Texture
       * 393 = Some sort of render pass (shadow?) nvm it just seems to be the first occurrence of many more textures
       * like it 257 = debug font texture
       */
  ...
  void renderDrawData(ImDrawData* drawData) {
    ...
    bd->cmdBuf->BindProgram(&bd->shaderProgram,
                            nvn::ShaderStageBits::VERTEX | nvn::ShaderStageBits::FRAGMENT); // bind main imgui shader
    ...
    setRenderStates(); // sets up the rest of the render state, required so that our shader properly gets drawn to the
                       // screen
    ...
    for (size_t i = 0; i < drawData->CmdListsCount; i++) {
      ...
      for (auto cmd : cmdList->CmdBuffer) {
        ...
        bd->cmdBuf->DrawElementsBaseVertex(nvn::DrawPrimitive::TRIANGLES, nvn::IndexType::UNSIGNED_SHORT, cmd.ElemCount,
